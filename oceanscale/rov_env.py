...
def _require_value(value: Any, name: str) -> Any:
    if value is None:
        raise RuntimeError(f"ROVEnv missing {name}")
    return value
...
class ROVEnv(gym.Env[Any, Any]):
    ...
    def __init__(
        ...
        self.target_pos = cast(FloatArray, np.asarray(target_pos, dtype=np.float32))
        ...
        self._t_pinv: FloatArray | None

        # Precompute pseudoinverse for wrench→thruster allocation
        if self._t_matrix is not None:
            self._t_pinv = cast(
                FloatArray,
                np.linalg.pinv(np.array(self._t_matrix, dtype=np.float64)).astype(np.float32),
            )
        else:
            self._t_pinv = None
    ...
    def _build(self) -> None:
        ...
        I_max = max(
            self.coeffs.get("Ix", 0.26), self.coeffs.get("Iy", 0.23), self.coeffs.get("Iz", 0.37)
        )
        ...
        self._prev_action_gpu = torch.zeros(self.n_envs, 6, device=self.device, dtype=torch.float32)
        self._prev_wrench_gpu = torch.zeros(self.n_envs, 6, device=self.device, dtype=torch.float32)
        ...
        self._fluid_positions: Any | None = None
        if self.use_fluid:
    ...
    def step(self, action: np.ndarray) -> tuple[Any, Any, Any, Any, dict[str, Any]]:
        ...
        self._prev_action_gpu = torch.tensor(action, device=self.device, dtype=torch.float32)
    ...
    def step_torch(
        ...
        if self._t_pinv is not None:
            ...
            u_cmd = torch.zeros(
                self.n_envs, self.n_thrusters, device=self.device, dtype=torch.float32
            )
            for i in range(min(6, self.n_thrusters)):
                u_cmd[:, i] = action[:, i]
        u_cmd *= self._thruster_gain_gpu.unsqueeze(1)
        self._u_cmd = cast(Any, wp.from_torch(u_cmd.contiguous(), dtype=wp.float32))
        body_f = _require_value(self.state_curr.body_f, "body_f")
        body_qd = _require_value(self.state_curr.body_qd, "body_qd")
    ...
    def _build_fluid_solver(self) -> None:
        ...
        self._fluid_mean_vel = np.mean(_wp_numpy(vel_field), axis=0).astype(np.float32)

        self._fluid_positions = wp.zeros(self.n_envs, dtype=wp.vec3, device=self.device)
    ...
    def _apply_ocean_current(self) -> None:
        ...
        """
        fluid = _require_value(self._fluid, "_fluid")
        body_q = wp.to_torch(_require_value(self.state_curr.body_q, "body_q"))
        positions = body_q[:, 0:3].clone()

        domain_size = fluid.domain_size
        half = domain_size / 2.0
        positions += half

        fluid_positions = cast(Any, wp.from_torch(positions.contiguous(), dtype=wp.vec3))
        self._fluid_positions = fluid_positions

        fluid_vel = fluid.sample_velocity_at(fluid_positions)
        fluid_vel_t = wp.to_torch(fluid_vel)

        body_qd = wp.to_torch(_require_value(self.state_curr.body_qd, "body_qd"))
        ...
        body_f = wp.to_torch(_require_value(self.state_curr.body_f, "body_f"))
        body_f[:, 0:3] += force_body

    def _apply_dr_current(self) -> None:
        """Apply domain-randomized ocean current as drag force (GPU-only)."""
        current_vec_t = wp.to_torch(_require_value(self.tier1.current_vec_arr, "current_vec_arr"))
        body_q = wp.to_torch(_require_value(self.state_curr.body_q, "body_q"))
        body_qd = wp.to_torch(_require_value(self.state_curr.body_qd, "body_qd"))
        quat = body_q[:, 3:7]
    ...
    @staticmethod
    def _rotate_to_world(quat: np.ndarray, vec_body: np.ndarray) -> np.ndarray:
    ...
    def _extract_quat(self) -> Any:
        """Extract quaternion from body_q without CPU transfer."""
        body_q_t = wp.to_torch(_require_value(self.state_curr.body_q, "body_q"))
        quat_dst = wp.to_torch(self._quat_buf)
        quat_dst.copy_(body_q_t[:, 3:7])
        return self._quat_buf

    def _get_body_state(self) -> tuple[NDArray[Any], NDArray[Any], NDArray[Any]]:
        """Return (position, quaternion, velocity) arrays, each (n_envs, ...)."""
        body_q = _wp_numpy(_require_value(self.state_curr.body_q, "body_q"))
        body_qd = _wp_numpy(_require_value(self.state_curr.body_qd, "body_qd"))
        pos = body_q[:, 0:3]
        quat = body_q[:, 3:7]
        vel = body_qd[:, 0:6]
        return pos, quat, vel
    ...
    def _get_obs_for(self, env_ids: np.ndarray) -> Any:
        ...
        obs[:, 15] = 2.0 * torch.asin(torch.clamp(-q[:, 0] * q[:, 2] + q[:, 1] * q[:, 3], -1, 1))
    ...
    def _compute_reward_with_components(
        ...
        pos, _quat, vel = self._get_body_state_torch()
    ...
    def _reset_indices(self, env_ids: np.ndarray) -> None:
        ...
        if self.init_pos_noise_std > 0 or self.init_yaw_noise_std > 0:
            pos_noise = self.np_random.uniform(
                -self.init_pos_noise_std, self.init_pos_noise_std, (n_reset, 3)
            ).astype(np.float32)
            yaw_offsets = self.np_random.uniform(
                -self.init_yaw_noise_std, self.init_yaw_noise_std, n_reset
            ).astype(np.float32)
        else:
