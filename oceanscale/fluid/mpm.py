...
class NewtonMPMFluid:
    ...
    def __init__(
        ...
        self.create_dam_break(
            ...
            fluid_region=(0.0, 0.0, 0.0, domain_size[0] * 0.5, domain_size[1] * 0.5, domain_size[2] * 0.5),
        ...
        self.model.mpm.young_modulus.fill_(1.0e6)
        self.model.mpm.yield_pressure.fill_(1.0e4)
    ...
    def get_particle_positions(self) -> wp.array:
        """Return current particle positions (n_particles, 3)."""
        return self.state_0.particle_q

    def get_particle_velocities(self) -> wp.array:
        """Return current particle velocities."""
        return self.state_0.particle_qd
        ...
        while len(points) < n_particles:
            ...
            for p in batch:
                if len(points) >= n_particles:
                    break
                points.append(center + p * sphere_r)

        points = np.array(points[:n_particles], dtype=np.float32)
