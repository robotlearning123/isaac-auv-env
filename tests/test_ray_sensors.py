...
def _make_flat_plane(z: float = 0.0, size: float = 100.0) -> wp.Mesh:
    """Create a flat plane mesh at given z height."""
    pts = np.array([
...
class TestRayDVL:
    ...
    def test_out_of_range(self):
        mesh = _make_flat_plane(z=0.0, size=1.0)
        dvl = RayDVL(mesh, n_beams=4, beam_angle=30.0, max_range=50.0)
        pos = np.array([0.0, 0.0, 1000.0], dtype=np.float32)
        result = dvl.measure(pos)
        assert result["altitude"] == 50.0
        assert result["valid"] is False

    def test_closer_means_shorter_range(self):
        mesh = _make_flat_plane(z=-30.0)
        dvl = RayDVL(mesh, n_beams=4, beam_angle=30.0, max_range=200.0)
        r_high = dvl.measure(np.array([0, 0, 0], dtype=np.float32))
        r_low = dvl.measure(np.array([0, 0, -20], dtype=np.float32))
...
class TestRaySonar:
    ...
    def test_empty_scene_max_range(self):
        mesh = _make_flat_plane(z=-100.0, size=1.0)
    ...
    def test_obstacle_detection(self):
        box = _make_box(center=np.array([10.0, 0.0, 0.0]), half_ext=2.0)
        ...
        assert np.any(center_rays < 15.0), "Box at x=10, sonar should see it at ~8m"

    def test_closer_obstacle_shorter_range(self):
        box_near = _make_box(center=np.array([5.0, 0.0, 0.0]), half_ext=1.0)
        box_far = _make_box(center=np.array([20.0, 0.0, 0.0]), half_ext=1.0)
    ...
    def test_ranges_positive(self):
        box = _make_box(center=np.array([5.0, 0.0, 0.0]), half_ext=1.0)
