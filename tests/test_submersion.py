...
class TestSubmersionDrag:
    def test_drag_when_submerged(self, model):
        _, drag = _single(model, z=-5.0, vz=1.0)
        assert drag[0, 2] < 0.0
    ...
    def test_drag_scales_with_submersion(self, model):
        _, drag_full = _single(model, z=-5.0, vz=1.0, h=1.0)
        _, drag_half = _single(model, z=0.0, vz=1.0, h=2.0)
    ...
    def test_zero_velocity_no_drag(self, model):
        _, drag = _single(model, z=-5.0, vz=0.0)
...
class TestSubmersionBatch:
    def test_multiple_links_at_different_depths(self, model):
        ...
        buoy, drag = model.compute(positions, velocities, heights, volumes)
